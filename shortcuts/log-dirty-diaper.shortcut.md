...
| | |
|---|---|
| **Siri phrase** | "Log a dirty diaper" |
| **Logs** | A diaper that was **both** wet and dirty (`pee: true, poop: true`) |
| **Inputs asked** | None |
| **Endpoint** | `POST {APP_URL}/api/events` |

This is the "both" case. `log-pee` covers wet-only, `log-poop` dirty-only;
this one is the single-phrase shortcut for the common combined change. The
`InboundEvent` diaper variant requires **at least one** of `pee`/`poop` true —
here both are true.

## `event` payload

```json
{ "type": "diaper", "pee": true, "poop": true }
```
...
curl -X POST "$APP_URL/api/events" \
  ...
  -d '{"client_uuid":"'"$(uuidgen)"'","source":"siri_shortcut","occurred_at":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","event":{"type":"diaper","pee":true,"poop":true}}'
# → { "ok": true, "event_id": "...", "say": "Logged. N wet, M dirty today." }
```
